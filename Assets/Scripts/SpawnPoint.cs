...
public class SpawnPoint : MonoBehaviour
{
    ...
    {
        if (Network.isServer)
            SpawnTotem(animalNames[0], 
                       animalNames.Length > 1 ? animalNames[1] : null,
                       animalNames.Length > 2 ? animalNames[2] : null);
        else
            networkView.RPC("SpawnTotem", RPCMode.Server, animalNames);
    }

    [RPC]
    public void SpawnTotem(string animalName1, string animalName2, string animalName3)
    {
        if (!Network.isServer)
